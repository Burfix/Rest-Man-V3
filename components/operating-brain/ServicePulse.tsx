  ...
  return (
    <div className="space-y-2">
      ...
      <div className="rounded-xl border border-stone-200 dark:border-stone-800/40 bg-stone-50 dark:bg-stone-900/50 p-4">
        ...
        {/* ── Provenance footer ──────────────────────────────────────── */}
        {provenance && (
          <div className="mt-3 border-t border-stone-200 dark:border-stone-800/40 pt-2">
            {provenance.isStale && provenance.source !== "no_connection" && (
              <p className="mb-1 rounded bg-amber-950/30 px-2 py-1 text-[10px] text-amber-400">
                {provenance.reason ?? "Revenue data may be stale"}
              </p>
            )}
            <p className="text-[10px] text-stone-500 dark:text-stone-600">
              {provenance.source === "live_micros" && "Live MICROS"}
              {provenance.source === "stale_fallback" && "MICROS (stale)"}
              {provenance.source === "manual_upload" && "Manual upload"}
              {provenance.source === "no_connection" && "No POS connection"}
              {provenance.source === "cached" && "Cached"}
              {provenance.source === "mock" && "Mock data"}
              {provenance.locRef && (
                <span className="ml-1 opacity-60">· loc {provenance.locRef}</span>
              )}
              {provenance.fetchedAt && (
                <span className="ml-1 opacity-60">
                  · synced {new Date(provenance.fetchedAt).toLocaleTimeString("en-ZA", { hour: "2-digit", minute: "2-digit" })}
                </span>
              )}
            </p>
          </div>
