...
export default function DataHealthWarning({ health, inventoryProvenance }: Props) {
  ...
  return (
    <div className={cn("rounded border px-4 py-3 space-y-2.5", meta.bg)}>
      ...
      {/* ── Inventory provenance footer ─────────────────────────── */}
      {inventoryProvenance && (
        <div className="pt-2 border-t border-stone-200 dark:border-stone-800/30">
          {inventoryProvenance.isStale && inventoryProvenance.source !== "no_connection" && (
            <p className="mb-1 rounded bg-amber-950/30 px-2 py-0.5 text-[9px] text-amber-400">
              {inventoryProvenance.reason ?? "Inventory data may be stale"}
            </p>
          )}
          <p className="text-[9px] text-stone-600 font-mono">
            INV · 
            {inventoryProvenance.source === "live_micros" && "MICROS IM"}
            {inventoryProvenance.source === "manual_upload" && "Manual count"}
            {inventoryProvenance.source === "stale_fallback" && "Stale"}
            {inventoryProvenance.source === "no_connection" && "Not configured"}
            {inventoryProvenance.fetchedAt && (
              <span className="ml-1 opacity-50">
                · {new Date(inventoryProvenance.fetchedAt).toLocaleTimeString("en-ZA", { hour: "2-digit", minute: "2-digit" })}
              </span>
            )}
          </p>
        </div>
