/**
 * services/reports/dailyReport.ts
 *
 * Core daily report builder + email sender.
 * Called by lib/scheduler/async-scheduler.ts when processing
 * `send_daily_report` async jobs.
 */
...
export async function sendDailyReport(targetSiteId: string | undefined, today: string): Promise<NextResponse> {
  const supabase = createServerClient() as any;

  const siteQuery = supabase.from("sites").select("id, name, city, target_labour_pct").eq("is_active", true);
  if (targetSiteId && targetSiteId !== "ALL") siteQuery.eq("id", targetSiteId);
  ...
  if (siteIds.length === 0) {
    logger.warn("[DailySync] No active sites â€” aborting email");
  ...
  // â”€â”€ Build per-store data â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  const storeData = siteList.map((site) => {
    ...
    return {
      ...
      tasks: tasks.map((t) => ({
        ...
        status: t.status,
        started_by: t.assigned_to ? (nameMap[t.assigned_to] ?? "â€”") : "â€”",
  ...
  // â”€â”€ 3. Send email â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  const recipient = process.env.DAILY_REPORT_EMAIL;
  if (!recipient) {
    logger.warn("[DailySync] DAILY_REPORT_EMAIL not set â€” skipping send");
    return NextResponse.json({ ok: true, email_sent: false, note: "DAILY_REPORT_EMAIL not configured" });
  }

  const resendKey = process.env.RESEND_API_KEY;
  if (!resendKey) {
    logger.warn("[DailySync] RESEND_API_KEY not set â€” skipping send");
  ...
  const subject = `Daily Report Â· ${storeLabel} Â· ${today}`;
...
<head>
  ...
  <title>Daily Ops Report Â· ${esc(date)}</title>
</head>
          ...
          <tr>
            <td style="padding-top:32px;border-top:1px solid #1f1f1f;margin-top:24px;">
              <p style="font-size:11px;color:#4b5563;text-align:center;margin:0;">
                Automated daily ops report Â· Generated ${esc(new Date().toISOString())}
              </p>
            </td>
          </tr>
...
function buildStoreBlock(s: any, date: string): string {
  ...
  const taskRows = s.tasks.map((t: any) => {
    ...
    const blockerCell = t.blocker_reason
      ...
      : esc(t.completion_comment || t.start_comment || "â€”");
    ...
    return `
        <tr style="border-bottom:1px solid #1f1f1f;">
          ...
          <td style="padding:6px 8px;color:#9ca3af;font-size:11px;text-align:right;">${t.duration_minutes ?? "â€”"}</td>
    ...
    : s.maintenance.issues.map((m: any) => {
        ...
        <div style="border:1px solid #2a2a2a;border-radius:6px;padding:10px 12px;margin-bottom:6px;">
          ...
          <div style="font-size:11px;color:#6b7280;">
            ${m.assigned_to ? `<span>Reported by: <span style="color:#9ca3af;">${esc(m.assigned_to)}</span></span>&nbsp;Â·&nbsp;` : ""}
            <span style="color:${statusColor};font-weight:600;">${esc((m.status ?? "").replace(/_/g, " "))}</span>
          </div>
          ${m.description ? `<div style="font-size:11px;color:#6b7280;margin-top:4px;">${esc(m.description.slice(0, 120))}${m.description.length > 120 ? "â€¦" : ""}</div>` : ""}
        </div>`;
      }).join("");
  ...
  const complianceContent = s.compliance.expired === 0
    ...
    : `<p style="font-size:12px;color:#ef4444;margin:0 0 6px 0;font-weight:700;">${s.compliance.expired} overdue item${s.compliance.expired > 1 ? "s" : ""}</p>` +
      s.compliance.items.map((ci: any) =>
        `<div style="font-size:11px;color:#9ca3af;padding:3px 0;">Â· ${esc(ci.name)} <span style="color:#6b7280;">(${esc(ci.category)})</span></div>`
      ).join("");
              ...
              <table width="100%" cellpadding="0" cellspacing="0">
                <tr>
                  <td>
                    ...
                    <div style="font-size:12px;color:#6b7280;margin-top:2px;">${esc(s.city)} Â· Daily Report Â· ${esc(date)}</div>
                  </td>
        ...
        <div style="padding:20px;">
          ...
          <table width="100%" cellpadding="0" cellspacing="0" style="margin-bottom:20px;">
            <tr>
              ...
              ${kpiCard("Labour %", f.labour_pct != null ? `${f.labour_pct}%` : "â€”", `Target: ${f.target_labour_pct}%`, labourColor)}
              ${kpiCard("Score", f.operating_score != null ? String(f.operating_score) : "â€”", f.score_grade ?? undefined)}
            </tr>
          </table>
          ...
          <div style="margin-bottom:20px;">
            ...
            <div style="border:1px solid #1a3a2a;border-radius:6px;padding:10px 12px;margin-bottom:6px;background:#0d1f17;">
              ...
              <div style="font-size:10px;color:#4b5563;">
                ${cd.owner ? `${esc(cd.owner)} Â· ` : ""}${cd.completed_at ? new Date(cd.completed_at).toLocaleTimeString("en-ZA", { hour: "2-digit", minute: "2-digit" }) : ""}
              </div>
            </div>`).join("")}
          </div>` : ""}
            ...
            <table width="100%" cellpadding="0" cellspacing="0" style="border-collapse:collapse;">
              ...
              <tbody>
                ...
                <tr style="border-bottom:1px solid #1f1f1f;">
                  ...
                  <td style="padding:6px 8px;text-align:right;font-size:11px;color:#9ca3af;">${g.completionRate != null ? g.completionRate + "%" : "â€”"}</td>
                  <td style="padding:6px 8px;text-align:right;font-size:11px;color:#9ca3af;">${g.onTimeRate != null ? g.onTimeRate + "%" : "â€”"}</td>
                  <td style="padding:6px 8px;text-align:right;font-size:11px;color:#9ca3af;">${g.tasksCompleted ?? "â€”"}/${g.tasksAssigned ?? "â€”"}</td>
