...
/**
 * Convert an InventoryIntelligence snapshot into the canonical DataProvenance contract.
 *
 * - imEnabled=true  → source is a MICROS IM sync; stale SLA = 60 min
 * - imEnabled=false → source is a manual stock count; no time-based SLA
 * - totalItems === 0 → no inventory configured for this site ("no_connection")
 */
export function inventoryToProvenance(
  intel: InventoryIntelligence | null,
  siteId: string,
  imEnabled: boolean,
): DataProvenance {
  if (!intel || intel.totalItems === 0) {
    return buildDataProvenance({
      source: "no_connection",
      fetchedAt: null,
      siteId,
      reason: "No inventory configured for this site",
    });
  }
  ...
  if (imEnabled) {
    return buildDataProvenance({
      source: "live_micros",
      fetchedAt: intel.lastSynced,
      staleAfterMinutes: 60,
      siteId,
      reason: !intel.lastSynced ? "MICROS IM sync not yet recorded" : undefined,
    });
  }

  // Manual stock count — no time-based SLA
  return buildDataProvenance({
