...
// ── DataProvenance bridge ───────────────────────────────────────────────────

/**
 * Convert a NormalizedSalesSnapshot into the canonical DataProvenance contract.
 *
 * Revenue stale SLA: 20 minutes (MICROS live data).
 * Manual uploads and forecasts are always "estimated" — no SLA applies.
 */
export function snapshotToProvenance(
  snap: NormalizedSalesSnapshot,
  siteId: string,
  locRef?: string | null,
): DataProvenance {
  return buildDataProvenance({
    source:
      snap.source === "micros" && snap.isLive  ? "live_micros"    :
      snap.source === "micros"                 ? "stale_fallback" :
      snap.source === "manual"                 ? "manual_upload"  :
      snap.source === "forecast"               ? "stale_fallback" :
      "no_connection",
    fetchedAt: snap.lastUpdatedAt,
    staleAfterMinutes: snap.source === "micros" ? 20 : undefined,
    locRef:   locRef ?? undefined,
    siteId,
    reason:
      snap.source === "forecast"               ? "No live POS data — using revenue forecast" :
      snap.source === "micros" && snap.isStale ? `MICROS data ${snap.freshnessMinutes ?? "?"} min old` :
      undefined,
  });
}
